...
function createRemoteRun(label = 'opponent') {
  const run = createRun({ mode: 'pvp', label });
...
function enterQueue() {
  ...
  queueMessage.textContent = '도전자와 연결 중...';
  clearQueueTimers();
  ensurePeer();
  ...
  state.queueTicker = setInterval(() => {
    ...
    queueMessage.textContent = `매칭 시도 중... AI 대체까지 ${(remaining / 1000).toFixed(1)}초`;
  }, 250);
  state.queueMatchInterval = setInterval(pollMatchmaking, 1400);
  pollMatchmaking();
  state.queueAIHandle = setTimeout(() => {
    if (state.peerConn) return;
    clearQueueTimers();
    queueMessage.textContent = '상대가 없어 적응형 AI로 대전합니다.';
    state.queueTimeout = setTimeout(() => startPvp({ ai: true }), 1200);
  }, fallback);
  hudEl.innerHTML = '<div>대전 상대를 찾는 중...</div>';
}
...
function startPvp({ ai = true } = {}) {
  ...
  state.opponentRun = ai ? createAIRun() : createRemoteRun();
  state.peerStarted = !ai && !!state.peerConn;
  state.paused = false;
  state.awaitingChoice = false;
  state.recentUnlocks = [];
  renderUnlockList();
...
function startNetworkPvp(startPayload) {
  if (state.peerStarted) return;
  state.peerStarted = true;
  resetOverlays();
  clearQueueTimers();
  queueOverlay.classList.add('hidden');
  const warmupMs = Math.max(0, startPayload.startAt - performance.now());
  state.mode = 'pvp';
  state.run = createRun({ mode: 'pvp', label: 'you', warmup: warmupMs / 1000 });
  const rivalName = state.peerMateNick || startPayload.nick || '상대';
  state.opponentRun = createRemoteRun(rivalName);
  state.paused = false;
  state.awaitingChoice = false;
  state.recentUnlocks = [];
  renderUnlockList();
  audio.startMusic();
  if (state.peerStatusInterval) clearInterval(state.peerStatusInterval);
  state.peerStatusInterval = setInterval(() => {
    if (!state.peerConn || !state.run) return;
    sendPeer({
      type: 'status',
      hp: state.run.player.hp,
      hpMax: state.run.player.hpMax,
      time: state.run.time,
...
function clearQueueTimers() {
  ...
  if (state.queueMatchInterval) {
    clearInterval(state.queueMatchInterval);
    state.queueMatchInterval = null;
  }
}
...
function pollMatchmaking() {
  if (!state.peer || state.peerConn || state.mode !== 'queue' || !state.peerId) return;
  state.peer.listAllPeers((ids) => {
    const target = ids.find((id) => id !== state.peerId);
    if (target) {
...
function connectToPeer(peerId) {
  if (!state.peer || state.peerConn) return;
  const conn = state.peer.connect(peerId, { metadata: { nick: getNickname() } });
  setupPeerConnection(conn);
}

function setupPeerConnection(conn) {
  state.peerConn = conn;
  state.peerMateId = conn.peer;
  conn.on('open', () => {
    queueMessage.textContent = '상대를 찾았습니다. 준비 중...';
    sendPeer({ type: 'hello', id: state.peerId, nick: getNickname() });
    maybeStartNetwork();
  });
  conn.on('data', handlePeerData);
  conn.on('close', tearDownPeerConnection);
  conn.on('error', tearDownPeerConnection);
}

function sendPeer(payload) {
  try {
    state.peerConn && state.peerConn.open && state.peerConn.send(payload);
  } catch (err) {
...
function handlePeerData(data) {
  ...
  if (data.type === 'hello') {
    state.peerMateId = data.id;
    state.peerMateNick = data.nick || '상대';
    if (!state.peerHello) {
      state.peerHello = true;
      sendPeer({ type: 'hello', id: state.peerId, nick: getNickname() });
    }
    maybeStartNetwork();
  }
  if (data.type === 'start') {
    state.peerMateNick = data.nick || state.peerMateNick;
    startNetworkPvp(data);
    return;
  }
  if (data.type === 'choice') {
    const choice = CHOICE_LOOKUP[data.id];
    if (choice && state.run) {
      applyChoice(choice, state.run);
    }
  }
  if (data.type === 'status' && state.opponentRun) {
    state.opponentRun.player.hp = data.hp;
    state.opponentRun.player.hpMax = data.hpMax;
    state.opponentRun.time = data.time;
    if (data.result && !state.run?.result) {
      endGame('상대가 먼저 쓰러졌습니다', 'win');
    }
  }
  if (data.type === 'result' && state.opponentRun) {
    state.opponentRun.player.hp = 0;
    state.opponentRun.time = data.time;
    if (!state.run?.result) {
      const outcome = data.outcome === 'loss' ? 'win' : 'loss';
      const title = outcome === 'win' ? '상대가 먼저 쓰러졌습니다' : '당신이 먼저 쓰러졌습니다';
      endGame(title, outcome);
    }
  }
}

function maybeStartNetwork() {
  if (!state.peerConn || !state.peerId || !state.peerMateId || state.peerStarted || !state.peerMateNick) return;
  const host = state.peerId > state.peerMateId;
  if (host) {
    const startAt = performance.now() + 1200;
    const payload = { type: 'start', startAt, nick: getNickname() };
    sendPeer(payload);
    startNetworkPvp(payload);
  }
}

function tearDownPeerConnection() {
  if (state.peerStatusInterval) {
    clearInterval(state.peerStatusInterval);
    state.peerStatusInterval = null;
  }
  if (state.peerConn && state.peerConn.open) {
    try {
      state.peerConn.close();
    } catch (e) {}
  }
  state.peerConn = null;
  state.peerMateId = null;
  state.peerMateNick = null;
  state.peerStarted = false;
  state.peerHello = false;
}
...
function update(delta) {
  ...
  if (state.mode === 'pvp' && state.opponentRun && !state.opponentRun.remote) {
  ...
  if (state.run.player.hp <= 0 && !state.run.result) {
    endGame('당신이 쓰러졌습니다', state.mode === 'pvp' ? 'loss' : 'finished');
  }
...
function presentChoices(run, opponentChoice = false) {
  ...
  choiceTitle.textContent = state.mode === 'pvp' ? '상대에게 걸 저주 선택' : '다음 저주를 선택하세요';
  choiceSubtitle.textContent =
    state.mode === 'pvp'
      ? '상대도 동시에 당신에게 줄 저주를 고릅니다.'
      : '모든 저주는 중첩됩니다. 순수한 보너스는 없습니다.';
  ...
  function commitChoice(choice) {
    ...
    if (state.mode === 'pvp') {
      if (state.peerConn) {
        sendPeer({ type: 'choice', id: choice.id });
      } else {
...
function drawHUD() {
  if (!state.run) {
    hudEl.innerHTML = '<p>모드를 선택해 시작하세요.</p>';
    return;
  }
  const run = state.run;
  const player = run.player;
  ...
  if (state.mode === 'pvp' && state.opponentRun) {
    ...
    opponentBlock = `
      <div class="meter">
        <span class="meter-label">상대</span>
        <div class="meter-track"><span style="width:${foePct * 100}%"></span></div>
        <span>${foe.hp.toFixed(0)}</span>
      </div>
      <div>상대 저주: ${state.opponentRun.player.choiceHistory.length}</div>
    `;
  }
  hudEl.innerHTML = `
    <div>모드: ${state.mode === 'pvp' ? '1대1 대전' : '싱글 로그라이크'}</div>
    <div>생존 시간: ${run.time.toFixed(1)}초</div>
    ...
    <div>다음 저주까지 ${Math.max(0, run.nextChoice).toFixed(1)}초</div>
...
function endGame(title, result) {
  ...
  if (state.mode === 'pvp') {
    ...
    gameOverDetail.textContent = `당신 ${time}초 · 상대 ${opponentTime}초 버팀`;
  } else {
    gameOverDetail.textContent = `${time}초 생존했습니다. 다시 도전할까요?`;
  }
  ...
  if (state.mode === 'pvp' && state.peerConn) {
    sendPeer({ type: 'result', outcome: result, time: state.run.time });
  }
...
logoutBtn.addEventListener('click', () => {
  ...
  accountStatsEl.innerHTML = '<p>게스트로 플레이 중입니다.</p>';
  renderUnlockList();
});

function getNickname() {
  if (state.account) return state.account.nickname;
...
function renderAccountStats() {
  if (!state.account) {
    accountStatsEl.innerHTML = '<p>게스트로 플레이 중입니다. 로그인하면 해금 상황을 저장합니다.</p>';
    renderUnlockList();
    return;
  }
  const data = state.accounts[state.accountKey];
  ...
  accountStatsEl.innerHTML = `
    ...
    <div>레벨: ${level} · 경험치: ${data.exp}</div>
    <div>1대1 전적: ${data.wins}/${data.losses} (${winRate}%)</div>
    <div>싱글 최고 생존: ${data.bestSolo.toFixed(1)}초</div>
...
function renderUnlockList() {
  ...
  if (!state.account) {
    unlockListEl.innerHTML = '<span>로그인하면 더 많은 저주를 해금할 수 있습니다.</span>';
    return;
  }
  if (!state.recentUnlocks.length) {
    unlockListEl.innerHTML = '<span>이번 라운드에서 새로 해금한 저주가 없습니다.</span>';
    return;
  }
